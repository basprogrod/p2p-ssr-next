...
const Home: FC<HomeProps> = ({ electionItems, current, other, initiator, init }) => {
  ...
  useEffect(() => {
    ...
    const init = electionItems.reduce<{ [field: number]: number }>((acc, cur, i) => {
      acc[i] = 0
      return acc
    }, {})

    const preparedData = numbers.reduce<{ [field: number]: number }>((acc, cur) => {
      if (!!acc[cur]) acc[cur]++
      else acc[cur] = 1
  ...
  return (
    <div className="Home">
      {electionItems.map((name, i) => (
        <button key={name} disabled={isVoted} data-election-id={i} onClick={sendData}>
          {name}
        </button>
      ))}
